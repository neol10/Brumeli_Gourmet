...
function getCompactActionButtons(order) {
    if (order.status === 'pendente') {
        ...
        return `<button class="kds-btn-done" onclick="event.stopPropagation(); changeOrderStatus('${order.id}', 'pronto'); " style="padding: 6px 12px; font-size: 0.8rem; border-radius: 4px; border:none; color:white; font-weight:bold; cursor:pointer; pointer-events:all;"><i class="ph ph-check-circle"></i> Pronto</button>`;
    } else if (order.status === 'pronto') {
        return `<button class="btn-secondary" onclick="event.stopPropagation(); changeOrderStatus('${order.id}', 'entregue'); " style="padding: 6px 12px; font-size: 0.8rem; border-radius: 4px; pointer-events:all;"><i class="ph ph-paper-plane-tilt"></i> Entregar</button>`;
    }
    return '';
}
...
function getActionButtons(order, customerInfo = {}) {
    if (order.status === 'pendente') {
        ...
        return `<button class="kds-btn-done" onclick="changeOrderStatus('${order.id}', 'pronto'); closeKdsModal();" style="flex:1; padding: 15px; font-size: 1.2rem; cursor: pointer; border: none; border-radius: var(--radius-md); font-weight: bold;"><i class="ph ph-check-circle"></i> Marcar como Pronto</button>`;
    } else if (order.status === 'pronto') {
        ...
        return `
            <div style="display:flex; gap:12px; flex-direction:column; width:100%;">
                ${waBtnHtml}
                <button class="btn-secondary" onclick="changeOrderStatus('${order.id}', 'entregue'); closeKdsModal();" style="padding: 14px; font-size: 1rem;"><i class="ph ph-paper-plane-tilt"></i> Entregar / Despachar</button>
            </div>`;
    }
    return '';
}
