...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadMenu() {
        ...
        try {
            ...
            categories.forEach(cat => {
                ...
                // Mapeamento pra criar o link correto da Navbar (ex `#classicas`, `#artesanais`)
                const catKeys = {
                    'Pizzas': { icon: 'ph-pizza', id: 'pizzas' },
                    'Calzones': { icon: 'ph-cooking-pot', id: 'calzones' },
                    'Calzone': { icon: 'ph-cooking-pot', id: 'calzones' },
                    'Sucos': { icon: 'ph-drop', id: 'sucos' },
                    'Bebidas': { icon: 'ph-wine', id: 'bebidas' }
                };
                const props = catKeys[cat.name] || { icon: 'ph-pizza', id: `cat-${cat.id}` };
                ...
                prods.forEach(prod => {
                    ...
                    <div class="glass-card product-card">
                        ...
                        <div class="card-content">
                            ...
                            <button class="btn-secondary open-detail-btn" data-name="${prod.name}" data-price="${prod.price}" ${disabledAttr}><i class="ph ph-plus"></i> Adicionar</button>
                        </div>
                    </div>`;
                });
    ...
    let cart = JSON.parse(localStorage.getItem('agenciaPizzas_Cart')) || [];
    let activeCoupon = JSON.parse(localStorage.getItem('agenciaPizzas_Coupon')) || null;
    let deliveryType = localStorage.getItem('agenciaPizzas_DeliveryType') || 'retirada'; // 'retirada' | 'entrega'
    ...
    function setDeliveryType(type) {
        deliveryType = type;
        localStorage.setItem('agenciaPizzas_DeliveryType', type);
    ...
    function updateCartUI() {
        ...
        localStorage.setItem('agenciaPizzas_Cart', JSON.stringify(cart));
        localStorage.setItem('agenciaPizzas_Coupon', JSON.stringify(activeCoupon));
    }
    ...
    function loadProfileData() {
        const savedProfile = JSON.parse(localStorage.getItem('agenciaPizzas_Profile'));
        if (savedProfile) {
    ...
    async function renderOrderHistory() {
        ...
        const savedOrders = JSON.parse(localStorage.getItem('agenciaPizzas_Orders')) || [];
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', async () => {
            ...
            if (deliveryType === 'entrega') {
                ...
                textoPedido += "\n📍 *Cliente irá retirar na pizzaria.*";
            }
            ...
            const storedOrders = JSON.parse(localStorage.getItem('agenciaPizzas_Orders')) || [];
            storedOrders.push({
            ...
            localStorage.setItem('agenciaPizzas_Orders', JSON.stringify(storedOrders));
    ...
    // 7. LÓGICA DE BUSCA Dinâmica (Sutil e Robusta)
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            sections.forEach(section => {
                ...
                products.forEach(prod => {
                    ...
                    if (name.includes(term) || desc.includes(term)) {
                        prod.style.display = 'flex'; // Mantém o layout de card
                ...
                // Se não houver produtos visíveis na categoria, esconde o título da seção também
                section.style.display = hasVisibleProduct ? '' : 'none';
            });

            // Opcional: Se nada for encontrado em nenhuma seção, mostrar mensagem
            const dynamicContainer = document.getElementById('dynamic-menu-container');
            ...
            if (term && totalVisible === 0) {
                if (!noResults) {
                    ...
                    msg.innerHTML = `<i class="ph ph-mask-sad" style="font-size:3rem; color:var(--text-muted);"></i><p style="margin-top:10px;">Nenhum item encontrado para "${esc(term)}"</p>`;
